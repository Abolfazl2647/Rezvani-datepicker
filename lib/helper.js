...
var daysInOffsetPrevMonth = exports.daysInOffsetPrevMonth = function daysInOffsetPrevMonth(offset, year, month, lang, disable) {
    var prevClass = disable ? ' disabled ' : '  prev-days ';
    var listOfDays = [];
...
var daysInOffsetNextMonth = exports.daysInOffsetNextMonth = function daysInOffsetNextMonth(offset, total, year, month, disable) {
    var nextClass = disable ? ' disabled ' : ' next-days ';
    var maxLenght = 42;
...
function Calender(year, month, LANG, disable) {

    // console.log('calendar = ', year,month,LANG);

    var defaultClass = disable ? ' disabled ' : 'current-days';

    var DAY = TODAY(LANG);
    var TODAY_DATE = THIS_YEAR(LANG) + "-" + THIS_MONTH(LANG) + '-' + DAY;
    ...
    var PREV_OFFSET = daysInOffsetPrevMonth(FIRST_DAY_WEEK, PREV_YEAR, PREV_MONTH, LANG);
    var NEXT_OFFSET = daysInOffsetNextMonth(FIRST_DAY_WEEK, TOTAL_DAYS_IN_MONTH, NEXT_YEAR, NEXT_MONTH);
    ...
    var all_days = PREV_OFFSET.concat(CURRENT_MONTH_DAYS).concat(NEXT_OFFSET);

    for (var c = 0; c < all_days.length; c++) {
...
var changeArryaToArrayObj = function changeArryaToArrayObj(array, year, month, extera_text) {
    var obj = {};
    var finalArray = [];
